...
class Model_products extends CI_Model
{
		...
		if($cat_id) {
			...
			if($user_id == 1) {
				...
				foreach($query->result_array() as $key => $value) {
					...
					if(is_array($category_ids) && in_array($cat_id, $category_ids)) {
						
						$value['image'] = $this->getProductImagePath($value['image']);
						$result[] = $value;
					}
				} 
				...
				$sql = "SELECT * FROM products active=1 ORDER BY id DESC";
				$query = $this->db->query($sql);
				...
				foreach ($query->result_array() as $k => $v) {
					...
					if(in_array($cat_id, $category_ids) && in_array($user_data['store_id'], $store_ids)) {
						$v['image'] = $this->getProductImagePath($v['image']);
						$data[] = $v;
					}
				}
	...
	private function getProductImagePath($image_path) {
		if (file_exists($image_path) && is_file($image_path)) {
			return $image_path;
		} 
		else {
			return 'assets/images/product_image/default_image.png';
		}
	}
